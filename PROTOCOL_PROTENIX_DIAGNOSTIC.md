...
## Checkpoint availability amendment before base-model neural outputs

The Protenix-v2 L4 run reached the official checkpoint download step and failed before loading model weights or producing any prediction because the upstream `protenix-v2.pt` URL returned HTTP 403. A bounded URL probe on 14 September 2026 showed the official `protenix-v2.pt` checkpoint blocked while `protenix_base_default_v1.0.0.pt` returned HTTP 206 for a one-byte range request. Use `protenix_base_default_v1.0.0` as the strongest reachable official Protenix model with the same 2021-09-30 training-data cutoff. Keep all inputs, seeds, recycles, diffusion steps, no-template setting and interpretation rules unchanged. This is an infrastructure fallback before inspecting base-model scientific outputs, not an alternate model selected after seeing predictions. Preserve the failed v2 archive as evidence.
